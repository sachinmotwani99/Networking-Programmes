...
{
  int bd,sd,ad; char buff[1024];
  struct sockaddr_in cliaddr,servaddr; socklen_t clilen; clilen=sizeof(cliaddr); bzero(&servaddr,sizeof(servaddr));
...
recv(ad,buff,sizeof(buff),0); printf("Message received is %s\n",buff);
}
